...
> **What works today (v6.0.4):** the three knobs above are wired and resolve via the documented precedence (CLI > env > config > default) for `scan` (v6.0.1), `costs` and `fix` (v6.0.2), `brainstorm` and `spec` (v6.0.3), plus `plan` and `review` (v6.0.4). The engine modules ship and are exercised by `runs list / show / gc / delete / doctor` and `run resume <id>` (lookup). The `--json` channel discipline is live across every CLI verb. Budget enforcement is live for any phase that runs through `runPhase` with a `BudgetConfig`.
>
