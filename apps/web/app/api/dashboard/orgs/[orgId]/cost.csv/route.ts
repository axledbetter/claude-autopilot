...
export async function GET(req: Request, { params }: RouteParams): Promise<Response> {
  ...
  if (!isValidUuid(p.orgId)) {
    return Response.json({ error: 'malformed_params' }, { status: 422 });
  }
  const url = new URL(req.url);
  const period = parsePeriod(url.searchParams.get('since'), url.searchParams.get('until'));
  if (!period) {
    return Response.json({ error: 'bad_period' }, { status: 422 });
  }

  const callerUserId = await resolveSessionUserId();
  if (!callerUserId) return Response.json({ error: 'unauthenticated' }, { status: 401 });
