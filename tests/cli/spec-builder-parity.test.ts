...
//
// v6.2.0 — builder-extraction parity for `spec` (per spec WARNING #4).
//
// See `tests/cli/scan-builder-parity.test.ts` for the test rationale.
...
import { runSpec, buildSpecPhase } from '../../src/cli/spec.ts';
...
function tmpProject(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'spec-parity-'));
}
...
async function captureRunSpec(cwd: string): Promise<CaptureResult> {
  const cap = await captureConsole(() => runSpec({ cwd, cliEngine: true }));
...
async function captureBuilder(cwd: string): Promise<CaptureResult> {
  const cap = await captureConsole(async () => {
    const built = await buildSpecPhase({ cwd, cliEngine: true });
...
describe('spec builder parity (v6.2.0 spec WARNING #4)', () => {
  it('runSpec and buildSpecPhase + runPhaseWithLifecycle produce identical channels', async () => {
    const cwdA = tmpProject();
    const cwdB = tmpProject();
    try {
      const a = await captureRunSpec(cwdA);
