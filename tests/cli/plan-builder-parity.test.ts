// tests/cli/plan-builder-parity.test.ts
//
// v6.2.0 — builder-extraction parity for `plan` (per spec WARNING #4).
...
import { runPlan, buildPlanPhase } from '../../src/cli/plan.ts';
...
function tmpProject(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'plan-parity-'));
}
...
/** Plan writes a timestamped file per invocation. The path differs between
 *  invocations (Date.now-derived); strip it from output for comparison. */
function normalizePlanPath(text: string): string {
  return text.replace(/\.guardrail-cache\/plans\/[^\s]+-plan\.md/g, '.guardrail-cache/plans/<TS>-plan.md');
}
function normalizePlanEvent(ev: unknown): unknown {
  const norm = normalizeEvent(ev);
  if (typeof norm !== 'object' || norm === null) return norm;
  const e = norm as Record<string, unknown>;
  // The plan output lives in the phase snapshot (artifacts) and event
  // payload references via `result` on phase.success — but events.ndjson
  // doesn't actually carry the path; the path lives only in the snapshot
  // file. Nothing to scrub here beyond what normalizeEvent already does.
  return e;
}
...
async function captureRunPlan(cwd: string): Promise<CaptureResult> {
  const cap = await captureConsole(() => runPlan({ cwd, cliEngine: true }));
...
async function captureBuilder(cwd: string): Promise<CaptureResult> {
  const cap = await captureConsole(async () => {
    const built = await buildPlanPhase({ cwd, cliEngine: true });
...
describe('plan builder parity (v6.2.0 spec WARNING #4)', () => {
  it('runPlan and buildPlanPhase + runPhaseWithLifecycle produce identical channels', async () => {
    const cwdA = tmpProject();
    const cwdB = tmpProject();
    try {
      const a = await captureRunPlan(cwdA);
      const b = await captureBuilder(cwdB);

      assert.equal(a.exitCode, b.exitCode, 'exit codes must match');
      assert.equal(
        normalizePlanPath(a.stdout),
        normalizePlanPath(b.stdout),
        'stdout must be identical (modulo timestamp in plan path)',
      );
      ...
      for (let i = 0; i < a.events.length; i++) {
        assert.deepEqual(
          normalizePlanEvent(a.events[i]),
          normalizePlanEvent(b.events[i]),
